...
export function LoginView() {
  ...
  return (
    <div className="h-screen">
      <Navbar />
      {!isTokenValid ? (
        <TokenStep setIsTokenValid={setIsTokenValid} />
      ) : (
        <CallsignStep />
      )}
    </div>
    ...
    <main className="px-10 flex flex-col gap-3 items-center justify-start h-full">
      ...
      <form className="flex flex-col items-center gap-3 w-full">
        <label className="flex flex-col gap-3 w-full">
          ...
          />
          {!isValid && (
            <span className="text-red-500">Koodi on virheellinen</span>
          )}
        </label>
        <button
...
function InstanceName({ children }: { children: React.ReactNode }) {
  return (
    <h1 className="text-white text-center font-oswald font-bold text-3xl">
      {children}
    </h1>
  );
}
