...
import { isProductListDTO, ProductListDTO } from "./dto/ProductListDTO";
import {
...
const PRODUCT_LIST_API_ENDPOINT = (product: string): string =>
  `/api/v1/instructions/${encodeURIComponent(product)}/en`;
const PRODUCT_API_ENDPOINT = "/api/v1/descriptions/en";
const PRODUCT_API_HEALTHCHECK_ENDPOINT = "/api/v1/healthcheck/services";

const EMPTY_CONTENT_SERVICE: ProductContentService =
  ProductContentServiceImpl.create("__empty", []);
...
export interface IContentService
  extends ObservableService<ContentServiceEvent> {
  /**
  ...
  addEventListener(
    ...
    listener: ObservableListener<ContentServiceEvent>,
  ): ObservableDestructor;
}
...
export class ContentService {
   ...
   * @private
   */
  private static _retryFailedLoadTimeout: number | NodeJS.Timeout | undefined =
    undefined;

  private static _health: ProductHealthCheckDTO | undefined;
  private static _state: ContentServiceState =
    ContentServiceState.UNINITIALIZED;
  ...
  private static _productContentServices: Map<string, ProductContentService> =
    new Map();
  ...
  private static _removeEventListener(
    ...
    if (event === ContentServiceEvent.PRODUCTS_CHANGED) {
      this._productChangedListeners = this._productChangedListeners.filter(
        (l) => l !== listener,
      );
    }
  }
  ...
  private static async _initialize() {
    ...
    const allUniqueProducts = Array.from(
      new Set([...productNames, ...allProductsInHealthyApi]),
    );
  ...
  private static async _loadAndSetProduct(name: string) {
    ...
    this._productContentServices.set(
      name,
      ProductContentServiceImpl.create(name, content),
    );

    this._currentProducts = [...this._currentProducts, name];
    this._notifyProductsChanged();
  }
  ...
  private static async _loadProductsByNames(names: readonly string[]) {
    ...
    const tasks = names.map(
      (name: string): Promise<void> =>
        this._loadAndSetProduct(name).catch((err) => {
          failed.push(name);
          throw err;
        }),
    );
  ...
  private static async _loadProductList(): Promise<ProductListDTO> {
    return await this._loadJsonResource<ProductListDTO>(
  ...
  private static async _loadProductHealthCheck(): Promise<ProductHealthCheckDTO> {
    return await this._loadJsonResource<ProductHealthCheckDTO>(
      PRODUCT_API_HEALTHCHECK_ENDPOINT,
      isProductHealthCheckDTO,
    );
  }

  private static async _loadJsonResource<T>(
    endpoint: string,
    isDTO: (obj: unknown) => obj is T,
  ): Promise<T> {
    ...
    if (!isDTO(data)) {
      console.error(
        `Invalid data received from server at ${endpoint}: ${JSON.stringify(
          data,
        )}`,
      );
  ...
  private static async _loadProduct(name: string): Promise<ProductDTO> {
    return await this._loadJsonResource<ProductDTO>(
      PRODUCT_LIST_API_ENDPOINT(name),
      isProductDTO,
    );
  }

  private static _loadProductContent(
    name: string,
    product: ProductDTO,
  ): readonly Content[] {
    try {
      ...
      if (!isArray(content)) {
        console.error(
          `Invalid content data for ${name}: ${JSON.stringify(content)}`,
        );
        return [];
      }
