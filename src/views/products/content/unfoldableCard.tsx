...
function prepareStep (
  ...
  const imageClassesFn = ProductContentRenderer.prepareClassName(value?.classes);
...
function prepareSteps (
  ...
  const fns = value.map(
    (item) => prepareStep(item)
  );
  return (context : RendererContext) => {
    return fns.map(
      ( fn ) : StepProps => fn(context)
    );
  };
}

export const unfoldableCard : RendererCompiler<BaseContent> = (content: UnfoldableCardContent) : CompiledRenderer => {
  const titleFn = ProductContentRenderer.compile(content.title);
  const contentFn = content.content ? ProductContentRenderer.compile(content.content) : () => undefined;
  const bodyFn = ProductContentRenderer.compile(content?.body);
  const stepsFn = content.steps ? prepareSteps(content.steps ?? []) : undefined;
  const stylingFn = content?.classes ? ProductContentRenderer.prepareClassName(content?.classes ?? []) : undefined;
  return (context : RendererContext) => <UnfoldableCard2
  ...
  >{bodyFn(context)}</UnfoldableCard2>;
}
