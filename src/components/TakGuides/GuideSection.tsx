...
export function GuideSection(props: GuideSectionProps) {
  ...
  const renderImage = (imageSrc: string, imageLink: string, imageClasses: string) => (
    <a href={imageLink} target="_blank" rel="noopener noreferrer" className={imageClasses.includes('align-right') ? 'ml-auto mr-4' : ''}>
      <img loading="lazy" src={imageSrc} className={`${imageClasses.replace('align-right', '').trim()}`} alt="Image" />
    </a>
  );

  return (
    <div className="flex w-[350px] max-w-full justify-between gap-2.5 mt-4 pb-2.5 bg-neutral-800 items-start self-stretch">
